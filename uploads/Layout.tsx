...
import { Clock, Users, UserCheck, Award, Shield, Monitor, LogOut, Settings, User, Menu, X, Loader2, Database, Download, Palette } from "lucide-react";
...
export default function Layout({ children }: LayoutProps) {
  ...
  return (
    <div className="flex min-h-screen w-full bg-background text-foreground">
        ...
        {isAuthenticated && admin && (
          <div className="flex items-center gap-2">
            ...
            <div className="hidden sm:block text-right">
              ...
              <p className="text-xs text-muted-foreground">{getPrivilegeLevelText(admin.level)}</p>
            </div>
          </div>
        )}
      </div>
      ...
      {/* Sidebar - Desktop */}
      <aside className="hidden lg:flex w-80 bg-card border-r border-border flex-col fixed h-full z-40">
        {/* Logo Section */}
        <div className="p-6 border-b border-border">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">Secure Time Tracking System</p>
            </div>
          </div>
        </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 mt-1">
                  <span className={`text-xs px-2 py-1 rounded-full ${getPrivilegeLevelColor(admin.level)}`}>
                    {getPrivilegeLevelText(admin.level)}
                  </span>
                </div>
              </div>
            </div>
        ...
        {/* Live Clock */}
        <div className="px-6 py-4 bg-primary/10 border-y border-border">
                      ...
                      className={({ isActive }) =>
                        `flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 ${
                          isActive
                            ...
                            : "text-foreground hover:bg-accent hover:border hover:border-border"
                        }`
                      }
                    >
              ...
              <li>
                <Dialog open={isSettingsOpen} onOpenChange={setIsSettingsOpen}>
                  <DialogTrigger asChild>
                    <button className="flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 text-foreground hover:bg-accent hover:border hover:border-border w-full">
        ...
        <div className="p-4 border-t border-border space-y-3">
          {/* Theme Toggle Button - Now cycles through all themes */}
          <Button
            variant="outline"
            onClick={cycleTheme}
            className="w-full flex items-center gap-3 px-4 py-3 rounded-xl border-border text-foreground hover:bg-accent hover:text-foreground transition-all duration-200 h-auto"
          >
            <Palette className="w-5 h-5" />
            <span className="font-medium flex-1 text-left">Theme</span>
            <div className="flex items-center gap-2">
              <span className="text-lg">{getCurrentThemeOption().emoji}</span>
              <span className="text-sm">{getCurrentThemeOption().label}</span>
            </div>
          </Button>
      ...
      {isMobileMenuOpen && (
        <>
          ...
          <aside className="lg:hidden fixed left-0 top-0 bottom-0 w-80 bg-card border-r border-border shadow-xl flex-col z-50 transform transition-transform duration-300 overflow-y-auto">
            <div className="p-6 border-b border-border flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-xl font-bold text-foreground">DTR System</h1>
                  <p className="text-xs text-muted-foreground mt-0.5">Secure Time Tracking</p>
                </div>
              </div>
              <Button
                variant="ghost"
                ...
                {canManageAdmins() && (
                  <li>
                    <Dialog open={isSettingsOpen} onOpenChange={setIsSettingsOpen}>
                      <DialogTrigger asChild>
                        <button 
                          className="flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 text-foreground hover:bg-accent hover:border hover:border-border w-full"
                          onClick={() => setIsMobileMenuOpen(false)}
                        >
                          <Settings className="w-5 h-5" />
                          <span className="font-medium">Settings</span>
      ...
      {/* Main Content */}
      <main className="flex-1 lg:ml-80 mt-16 lg:mt-0 min-h-screen bg-background">
        <div className="p-4 lg:p-6 lg:pr-8">
          <div className="bg-card rounded-xl border border-border shadow-sm min-h-[calc(100vh-2rem)]">
            {children}
          </div>
        </div>
      </main>
