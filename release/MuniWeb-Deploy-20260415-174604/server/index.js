...
function mergeConfigWithEnv(fileConfig) {
  ...
  return {
    database: {
      host: process.env.MYSQL_HOST || baseConfig.database?.host || 'localhost',
      user: process.env.MYSQL_USER || baseConfig.database?.user || 'root',
      password: process.env.MYSQL_PASSWORD || baseConfig.database?.password || '',
      database: process.env.MYSQL_DATABASE || baseConfig.database?.database || 'bless_dtr_test',
      port: parseInt(process.env.MYSQL_PORT || `${baseConfig.database?.port || 3306}`, 10)
    },
    export: {
      path: process.env.EXPORT_PATH || baseConfig.export?.path || 'exports'
    }
  };
}
...
function getSessionIdFromRequest(req) {
  return req.headers['x-session-id'] || req.headers['authorization']?.split(' ')[1] || `${req.ip}-${req.headers['user-agent']}`;
}
...
function loadConfig() {
  try {
    if (fs.existsSync(CONFIG_FILE_PATH)) {
      ...
      return mergeConfigWithEnv(fileConfig);
    } else {
...
app.get('/api/settings', requireAdminLevel(3), async (req, res) => {
  try {
...
// Update settings
app.post('/api/settings', requireAdminLevel(3), async (req, res) => {
  try {
...
app.post('/api/noters', requireAdminLevel(2), async (req, res) => {
  let connection;
  try {
...
// Add new official
app.post('/api/officials', requireAdminLevel(2), async (req, res) => {
  let connection;
  try {
...
// Update official
app.put('/api/officials/:id', requireAdminLevel(2), async (req, res) => {
  let connection;
  try {
    const { id } = req.params;
    const { name, position, signatory } = req.body;
...
// Delete official
app.delete('/api/officials/:id', requireAdminLevel(2), async (req, res) => {
  let connection;
  try {
    const { id } = req.params;
...
app.put('/api/noters/:id', requireAdminLevel(2), async (req, res) => {
  let connection;
  try {
    const { id } = req.params;
    const { name, position, office, signatory } = req.body;
...
app.delete('/api/noters/:id', requireAdminLevel(2), async (req, res) => {
  let connection;
  try {
    const { id } = req.params;
...
app.post('/api/biometrics', requireAdminLevel(3), async (req, res) => {
  let connection;
  try {
...
app.put('/api/biometrics/:id', requireAdminLevel(3), async (req, res) => {
  let connection;
  try {
    const { id } = req.params;
    const { name, ip_address, port, active } = req.body;
...
app.delete('/api/biometrics/:id', requireAdminLevel(3), async (req, res) => {
  let connection;
  try {
    const { id } = req.params;
...
// Add new admin
app.post('/api/admins', requireAdminLevel(3), async (req, res) => {
  let connection;
  try {
...
// Update admin
app.put('/api/admins/:id', requireAdminLevel(3), async (req, res) => {
  let connection;
  try {
    const { id } = req.params;
    const { name, username, password, level } = req.body;
...
// Delete admin
app.delete('/api/admins/:id', requireAdminLevel(3), async (req, res) => {
  let connection;
  try {
    const { id } = req.params;
...
app.post('/api/biometrics/check-status', requireAdminLevel(3), async (req, res) => {
  try {
...
async function startServer() {
  try {
    ...
    console.error('❌ Startup failed: Unable to connect to database.', error.message);
    process.exit(1);
  }
}
