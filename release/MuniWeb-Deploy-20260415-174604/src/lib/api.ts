...
function authFetch(input: RequestInfo | URL, init: RequestInit = {}) {
  ...
  return fetch(input, {
    ...init,
    headers: getAuthHeaders(incomingHeaders)
  });
}
...
export const api = {
  ...
  async employeesCount() {
    const response = await authFetch(`${API_URL}/api/employees/count`);
    if (!response.ok) throw new Error('Failed to fetch employee count');
    return response.json();
  },

  async attendance(date: string) {
    const response = await authFetch(`${API_URL}/api/attendance/${date}`);
  ...
  async getEmployeeById(id: number) {
    const response = await authFetch(`${API_URL}/api/employees/${id}`);
...
async updateEmployee(id: number, employeeData: any) {
  ...
  async noters() {
    const response = await authFetch(`${API_URL}/api/noters`);
  ...
  async officials() {
    const response = await authFetch(`${API_URL}/api/officials`);
  ...
  async biometrics() {
    const response = await authFetch(`${API_URL}/api/biometrics`);
  ...
  async updateDTR(id: number, dtrData: {
    ...
    const response = await authFetch(`${API_URL}/api/dtr/${id}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  async importDTR(data: {
    ...
    const response = await authFetch(`${API_URL}/import-dtr`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  async importDTRFile(formData: FormData) {
    const response = await authFetch(`${API_URL}/import-dtr-file`, {
      method: 'POST',
      body: formData,
    });
  ...
  async exportDTR(exportData: {
    ...
    const response = await authFetch(`${API_URL}/api/export-dtr`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  async generatePrintPDF(printData: {
    ...
    const response = await authFetch(`${API_URL}/api/dtr/generate-print-pdf`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  async previewDTR(previewData: {
    ...
    const response = await authFetch(`${API_URL}/api/dtr/preview`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  async generateExcelPreview(previewData: {
    ...
    const response = await authFetch(`${API_URL}/api/dtr/generate-excel-preview`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  async getEmployeePositions() {
    const response = await authFetch(`${API_URL}/api/positions`);
  ...
  // Get offices
  async getOffices() {
    const response = await authFetch(`${API_URL}/api/offices`);
...
async getOfficialPositions() {
  const response = await authFetch(`${API_URL}/api/officials/positions`);
...
async updateOfficial(id: number, officialData: { name: string; position: string; signatory: string }) {
  const response = await authFetch(`${API_URL}/api/officials/${id}`, {
    method: 'PUT',
    headers: {
      'Content-Type': 'application/json',
    },
...
async updateNoter(id: number, noterData: { 
  ...
  const response = await authFetch(`${API_URL}/api/noters/${id}`, {
    method: 'PUT',
    headers: {
      'Content-Type': 'application/json',
    },
...
async updateBiometric(id: number, biometricData: {
  ...
  const response = await authFetch(`${API_URL}/api/biometrics/${id}`, {
    method: 'PUT',
    headers: {
      'Content-Type': 'application/json',
    },
...
async checkUnimportedDTRs(employeeId: number) {
  const response = await authFetch(`${API_URL}/api/check-unimported-dtrs/${employeeId}`);
...
async massGeneratePrintPDF(printData: {
  ...
  const response = await authFetch(`${API_URL}/api/dtr/mass-generate-print-pdf`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
...
async importSingleDTR(importData: {
  ...
  if (importData.source === 'file' && importData.file) {
    ...
    const response = await authFetch(`${API_URL}/import-single-dtr`, {
      method: 'POST',
      body: formData,
    });
...
async checkBiometricStatus(device: { ip_address: string; port: number }) {
  const response = await authFetch(`${API_URL}/api/biometrics/check-status`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
...
addDTR: async (dtrData: {
  ...
  const response = await authFetch(`${API_URL}/api/dtr`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
...
async bulkUpdateSchedule(data: {
  ...
  const response = await authFetch(`${API_URL}/api/employees/bulk-schedule`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
...
async bulkUpdateScheduleOverrides(data: {
  ...
  const response = await authFetch(`${API_URL}/api/employees/bulk-schedule-overrides`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
  ...
  async getScheduleOverrides(employeeId: number) {
    const response = await authFetch(`${API_URL}/api/employees/${employeeId}/overrides`);
    if (!response.ok) throw new Error('Failed to fetch schedule overrides');
    return response.json();
  },

  async deleteScheduleOverride(id: number) {
    const response = await authFetch(`${API_URL}/api/employees/overrides/${id}`, {
      method: 'DELETE',
    });
  ...
  async getGroupedScheduleOverrides(employeeId: number) {
    const response = await authFetch(`${API_URL}/api/employees/${employeeId}/overrides-grouped`);
