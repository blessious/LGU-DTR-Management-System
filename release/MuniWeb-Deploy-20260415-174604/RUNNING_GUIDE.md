...
4.  For deployment packages, generate a sanitized SQL first (recommended):
  ```powershell
  npm run sanitize:sql
  ```
  This creates `database.deployment.sql` with table structures and a default deployment admin only.
5.  Click on the database, go to the **Import** tab.
6.  Choose `database.deployment.sql` for customer deployments.
7.  Use `database.sql` only for internal/dev environments where full sample data is intentionally needed.

### 2. Configure Environment Variables
You need to tell the app where the database and server are located.

#### Recommended: Guided Setup (No manual JSON editing)
Run this from PowerShell in the project root:
```powershell
powershell -ExecutionPolicy Bypass -File .\setup.ps1
```
This writes both `.env` and `server/config.json`.
...
On Windows, run this after installation to finalize `pywin32`:
```bash
...
4.  Open your browser to `http://localhost:8080` to use the app.
...
## 🔐 First Login (Deployment Package)

If you imported `database.deployment.sql`, login with:
...
3.  **Access the URL**: Other PCs can then open `http://YOUR_IP:8080` in their browser.
4.  **Firewall**: Ensure Port **5000** (Backend) and **8080** (Frontend) are allowed through the Windows Firewall.
...
## 📦 Build a Client Release Package

To produce a deployment-safe ZIP for customer installation:
...
Output is created under `release/`.

The package includes `database.deployment.sql` and excludes local/sensitive files like `.env`, `server/config.json`, `database.sql`, uploads, exports, and dependency folders.
